...
export default function MonthlyReportPage() {
  ...
  const chartData = useMemo(() => {
    return Array.from({ length: 30 }, (_, i) => {
      ...
      const day = i + 1;
      const isFuture = (parseInt(selectedMonth) > currentMonth) || 
                       (parseInt(selectedMonth) === currentMonth && day > new Date().getDate());
      
      // eslint-disable-next-line react-hooks/purity
      const generatedCost = isFuture ? 0 : Math.floor(Math.random() * 18000) + 4000;
      
      return {
        day: `${day}`,
        cost: generatedCost,
        kwh: Number((generatedCost / 1500).toFixed(2)),
        isFuture: isFuture
      };
    });
  }, [selectedMonth]);
            ...
            <div className="space-y-1">
              ...
              {isHistoryOpen && (
                <div className="ml-9 space-y-1 pr-2">
                  ...
                  <button onClick={() => {router.push("/history/monthly"); setIsSidebarOpen(false);}} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-blue-600 font-bold bg-blue-100/50 rounded-lg transition text-left">Monthly Report</button>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="flex items-center justify-between bg-white px-4 md:px-8 py-5 border-b border-gray-100 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg md:text-2xl font-bold text-blue-900 truncate">Monthly Analysis</h2>
          </div>
        ...
        <div className="p-4 md:p-8 flex-1 space-y-6 md:space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative flex items-center bg-white rounded-xl px-4 py-2.5 border border-gray-200 shadow-sm w-full sm:w-auto">
              ...
              <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-transparent text-sm font-bold text-gray-700 outline-none cursor-pointer appearance-none pr-8 w-full">
          ...
          <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-50 relative overflow-hidden">
            <div className="relative z-10">
              <p className="text-[10px] font-bold text-gray-400 mb-4 md:mb-2 uppercase tracking-[0.2em]">Total Usage & Cost - {monthOptions[parseInt(selectedMonth)]}</p>
          ...
          <div className="bg-white p-4 md:p-8 rounded-2xl shadow-sm border border-gray-50">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 md:mb-10 gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Data for {monthOptions[parseInt(selectedMonth)]}</p>
              </div>
              ...
              {mounted && (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey={activeUnit} radius={[4, 4, 0, 0]} barSize={window.innerWidth < 768 ? 6 : 12}>
